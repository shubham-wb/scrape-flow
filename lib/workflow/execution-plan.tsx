    ...
    for (let phase = 2;
        ...
        for (const currentNode of nodes) {
            if (planned.has(currentNode.id)) {
                // node already placed in execution plan 
                continue;
            }
...
function getInvalidInputs(node: AppNode, edges: Edge[], planned: Set<string>) {
    ...
    for (const input of inputs) {
        ...
        if (requiredInputProvidedByVisitedOutput) {
            //the inputs is required and wew have a valid value for it 
            ...
            if (!inputLinkedToOutput && inputLinkedToOutput && planned.has(inputLinkedToOutput)) {
                continue
            }
        }
